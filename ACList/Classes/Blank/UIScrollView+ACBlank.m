...
@interface UIView (ACConstraintBasedLayoutExtensions)
- (NSLayoutConstraint *)equallyRelatedConstraintWithView:(UIView *)view attribute:(NSLayoutAttribute)attribute;
@end

@interface ACBlankView : UIView
...
@property (nonatomic, readonly) ACBlankView *blankView;
@property (strong, nonatomic) ACBlank *model;
@end
@implementation UIScrollView (ACBlank)
...
- (ACBlankView *)blankView {
    ACBlankView *view = objc_getAssociatedObject(self, kBlankView);
    if (!view) {
        view = [ACBlankView new];
        view.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
        view.hidden = YES;
        
        view.tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(ac_didTapContentView:)];
...
- (ACBlank *)model {
    ACBlank *model = objc_getAssociatedObject(self, _cmd);
    if (!model) {
        model = [ACBlank new];
        [self setModel:model];
    }
...
- (void)reloadBlankData {
    [self ac_reloadBlankData];
}

#pragma mark - Reload APIs (Private)

- (void)ac_reloadBlankData {
    if (![self ac_canDisplay]) return;
    if ([self ac_itemsCount] == 0) {
        ACBlankView *view = self.blankView;
...
@interface ACBlankView ()
@end
@implementation ACBlankView
